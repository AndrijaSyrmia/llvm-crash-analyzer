...
std::string crash_analyzer::MemoryWrapper::ReadFromMemory(uint64_t addr, uint32_t byte_size, lldb::SBError& error)
{
    ...
    {
        SS << std::hex << this->ChangedMemoryAdresses[addr];
        SS >> StrVal;
    ...
    else if(this->Dec != nullptr)
    {
        ...
        LLVM_DEBUG(llvm::dbgs() << "Addressing unchanged location: " << addr <<  ":" << Val <<"\n";);
        SS << std::hex << Val;
        SS >> StrVal;
...
void crash_analyzer::MemoryWrapper::changeValue(uint64_t addr, std::string val)
{
...
void crash_analyzer::MemoryWrapper::dumpChangedMemory()
{
    std::stringstream SS;
    std::string str;
    LLVM_DEBUG(llvm::dbgs() << "Changed locations:\n";);
    for(auto& MA : this->ChangedMemoryAdresses)
    {
        // SS << std::hex << MA.first;
        // SS >> str;
        LLVM_DEBUG(llvm::dbgs() << "\t" << MA.first << " : " << MA.second << "\n";);
    }
}